...
export const useWorkerServices = (workerId?: string) => {
  ...
  const fetchWorkerServices = async (id?: string) => {
    try {
      setLoading(true);
      const url = 'https://api.xirfadsan.com/api/staff/services/all';
  ...
  const assignServices = async (workerId: string, subServiceIds: string[]) => {
    try {
      const res = await fetch(
        `https://api.xirfadsan.com/api/staff/assign-services/${workerId}`,
        {
          method: 'PUT',
  ...
  const getWorkerServices = async (workerId: string) => {
    try {
      const res = await fetch(
        `https://api.xirfadsan.com/api/staff/services/${workerId}`
      );
