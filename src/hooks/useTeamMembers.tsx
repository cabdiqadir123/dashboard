...
export const useTeamMembers = () => {
  ...
  const fetchTeamMembers = async () => {
    try {
      ...
      const res = await axios.get("https://api.xirfadsan.com/api/member/allNew");
      ...
      const members = membersArray.map((member: any) => ({
        ...
        image: `https://api.xirfadsan.com/api/member/image/${member.id}`, // attach image URL
      }));
  ...
  const createTeamMember = async (teamMember: { name: string; linkedin_profile: string; role: string; image: string | File | null; is_active: boolean; created_at: string }) => {
    try {
      ...
      const res = await axios.post('https://api.xirfadsan.com/api/member/addNew', formData, {
  ...
  const updateTeamMember = async (id: string, updates: Partial<TeamMember>) => {
    try {
      const res = await axios.put(
        `https://api.xirfadsan.com/api/member/updateNew/${id}`,
        updates,
        { headers: { 'Content-Type': 'multipart/form-data' } }
      );
  ...
  const deleteTeamMember = async (id: string) => {
    try {
      ...
      const res = await axios.post("https://api.xirfadsan.com/api/member/delete", {
        testimonial_id: id
      });
